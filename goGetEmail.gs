...
/*/
function getPinnedRepos(html){ //this function checks for pinned repose which are not forked, and returns those. 
  ...
  return JSON.parse('['+pinnedArr.toString()+']');
}
...
function doGet(a){ //this function takes JSONP data from a http request. the JSONP data is a github profile URL.
  ...
  var repo = getPinnedRepos(resp); //extract the path of the first repository. We need this path because we extract the email adddress from repository commits
  
  if(repo[0] != ''){ //if a repository exists
    var email = getEmail(repo[0]); //fetch the email with another function
